...
class HandController:
    def __init__(self):
        ...
        self.GRAB_THRESHOLD = 0.35
        self.RELEASE_THRESHOLD = 0.70
    ...
    def process(self, landmarks, handedness_score, w, h):
        ...
        # Use Palm Center (average of 5 points for stability)
        wrist = landmarks[0]
        palm_x = (wrist.x + landmarks[5].x + landmarks[9].x + landmarks[13].x + landmarks[17].x) / 5.0
        palm_y = (wrist.y + landmarks[5].y + landmarks[9].y + landmarks[13].y + landmarks[17].y) / 5.0
        
        # Sensitivity mapping
        clamped_x = max(self.MARGIN, min(1 - self.MARGIN, palm_x))
        clamped_y = max(self.MARGIN, min(1 - self.MARGIN, palm_y))
        ...
        for tip_idx in fingertips:
            ...
            dist = np.hypot(tip.x - palm_x, tip.y - palm_y)
            distances.append(dist)
        
        avg_dist = np.mean(distances)
        hand_size = np.hypot(landmarks[9].x - wrist.x, landmarks[9].y - wrist.y) + 1e-6
        curl_ratio = avg_dist / hand_size
        
        thumb_to_index = np.hypot(thumb_tip.x - landmarks[8].x, thumb_tip.y - landmarks[8].y)
        thumb_curl = thumb_to_index / hand_size
        final_curl = (curl_ratio + thumb_curl) / 2.0
        
        # Debouncing
        if final_curl < self.GRAB_THRESHOLD:
            self.debounce_counter = min(self.DEBOUNCE_FRAMES, self.debounce_counter + 1)
        elif final_curl > self.RELEASE_THRESHOLD:
...
# --- DEBUG VISUALIZATION ---
def draw_hand_debug(screen, landmarks, w, h, margin=0.25):
    def map_coord(lm):
        clamped_x = max(margin, min(1 - margin, lm.x))
        clamped_y = max(margin, min(1 - margin, lm.y))
        active_width = 1 - (2 * margin)
        normalized_x = (clamped_x - margin) / active_width
        normalized_y = (clamped_y - margin) / active_width
        return (int(normalized_x * w), int(normalized_y * h))
    ...
    for start, end in connections:
        start_pos = map_coord(landmarks[start])
        end_pos = map_coord(landmarks[end])
        pygame.draw.line(screen, (0, 255, 255), start_pos, end_pos, 2)
    
    for i, lm in enumerate(landmarks):
        pos = map_coord(lm)
        color = (255, 0, 0) if i in [4, 8, 12, 16, 20] else (0, 255, 0)
    ...
    palm_center = PalmPoint(palm_x, palm_y)
    palm_pos = map_coord(palm_center)
    pygame.draw.circle(screen, (255, 255, 0), palm_pos, 10)
    pygame.draw.circle(screen, (0, 0, 0), palm_pos, 11, 2)
    font = pygame.font.SysFont("Arial", 18, bold=True)
    text = font.render("PALM CENTER", True, (255, 255, 0))
    text_bg = font.render("PALM CENTER", True, (0, 0, 0))
    screen.blit(text_bg, (palm_pos[0] + 12, palm_pos[1] - 22))
    screen.blit(text, (palm_pos[0] + 11, palm_pos[1] - 23))

# --- MAIN LOOP ---
def main():
    pygame.init()
    screen = pygame.display.set_mode(WINDOW_SIZE)
    pygame.display.set_caption("Factory Frenzy - Fixed")
    ...
    while True:
        ...
        if show_camera:
            ...
            lbl = pygame.font.SysFont("Arial", 30).render(
                status, True, (0, 255, 0) if gamma_active else (255, 255, 0)
            )
            screen.blit(lbl, (20, 60))
            
            # DIAGNOSTIC: Show raw landmark values
            if result.hand_landmarks:
                wrist = result.hand_landmarks[0][0]
                lm5 = result.hand_landmarks[0][5]
                palm_x_raw = (wrist.x + result.hand_landmarks[0][5].x + result.hand_landmarks[0][9].x + result.hand_landmarks[0][13].x + result.hand_landmarks[0][17].x) / 5.0
                diag_font = pygame.font.SysFont("Arial", 20)
                diag1 = diag_font.render(f"Wrist X: {wrist.x:.3f} | Palm X: {palm_x_raw:.3f}", True, (255, 255, 255))
                diag2 = diag_font.render(f"Pointer: {pointer}", True, (255, 255, 255))
                screen.blit(diag1, (20, 100))
                screen.blit(diag2, (20, 130))
                
                draw_hand_debug(screen, result.hand_landmarks[0], WINDOW_SIZE[0], WINDOW_SIZE[1])
            if pointer:
                # Draw HUGE cursor so we can see alignment clearly
                pygame.draw.circle(screen, (255, 0, 255), pointer, 20, 3)  # Magenta outline
                pygame.draw.line(screen, (255, 0, 255), (pointer[0]-25, pointer[1]), (pointer[0]+25, pointer[1]), 3)
                pygame.draw.line(screen, (255, 0, 255), (pointer[0], pointer[1]-25), (pointer[0], pointer[1]+25), 3)
        else:
